...
from model import FewShotModel, FewShotModel2, FewShotModel3
from model import Fewshot_EJK
from model import resnet

from src.test_dataset import CUB as Test_Dataset
from src.test_sampler import Test_Sampler
" User input value "
TOTAL = 100000  # total step of training
    ...
    with torch.no_grad():
        for i in range(TEST_SIZE):
            for episode in test_loader:
                ...
                """
                shot_output = model(data_shot)
                query_output = model(data_query)
                prototypes = shot_output.reshape(5,5,-1).mean(dim=1)
                logits = square_euclidean_metric(query_output, prototypes)
...
def train(args):
    ...
    for i in range(TOTAL):
        for episode in data_loader:
            ...
            shot_output = model(data_shot)      # 25*40000
            query_output = model(data_query)    # 20* 40000
            
            #  prototypes = torch.stack([shot_output[idx_list].mean(0) for idx_list in range(5)])
            prototypes = shot_output.reshape(5,5,-1).mean(dim=1)
            logits = square_euclidean_metric(query_output, prototypes)
        ...
        if (i+1) % SAVE_FREQ == 0:
            PATH = 'check_model2/%d_%s.pth' % (i + 1, args.name)
...
if __name__ == '__main__':
    ...
    if not os.path.isdir('check_model2'):
        os.mkdir('check_model2')
